...
import { getTopSellingProducts, getAllProducts, getAllCategories, getProductsByCategory } from "@/service/productService";
...
export default async function Home() {
  ...
  if (session) {
    try {
      [topSellingProducts, allProducts, categories] = await Promise.all([
        getTopSellingProducts(),
        getAllProducts(),
        getAllCategories()
      ]);

      const categoryProductPromises = categories.map(cat =>
      ...
      categories.forEach((cat, index) => {
        categorizedData[cat.categoryId] = results[index] ?? [];
      });
    } catch (err) {
  ...
  const bestSellers = topSellingProducts?.slice(0, 4);
  const heroStrip = allProducts?.slice(0, 3);

  return (
    <div className="bg-[#fafafa]">
      <LandingHeroSectionComponent miniProducts={heroStrip} token={session}/>
      <LandingBestSellerSectionComponent items={bestSellers} token={session}/>
      <LandingEssentialComponent
        token={session}
        categories={categories}
        products={allProducts}
        preFetchedData={categorizedData}
      />
